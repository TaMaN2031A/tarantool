...
static inline int
interval_usec(const struct datetime_interval *ival)
{
	return ival->nsec / 1000;
}

static inline int
interval_msec(const struct datetime_interval *ival)
{
	return ival->nsec / 1000000;
}
...
static inline int
interval_weeks(const struct datetime_interval *ival)
{
	return interval_seconds(ival) / (7 * SECS_PER_DAY);
}
